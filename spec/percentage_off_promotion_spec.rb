...
describe PercentageOffPromotion do
  it 'deducts 10% when it exceeds the eligibility of the rule' do
    ...
    expect(percentage_off_promo.discounted_basket).to eq [discounted_chair, discounted_chair, discounted_table]
  end
  ...
  it 'deducts 10% when it meets exactly the eligibility of the rule' do
    ...
    expect(percentage_off_promo.discounted_basket).to eq [discounted_fake_item, discounted_fake_item, discounted_fake_item]
  end
